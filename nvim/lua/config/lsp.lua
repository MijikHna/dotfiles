...
keymap.set("n", "gp", function() vim.diagnostic.jump({ count = -1 }) end, { desc = "[G]o to [P]revious Diagnostics" })
...
vim.api.nvim_create_user_command("LspClients", function()
  ...
  vim.print(#names > 0 and ("Attached LSPs: " .. table.concat(names, ", ")) or "No LSP clients attached")
end, {})
...
vim.api.nvim_create_user_command("LspInfo", function()
  ...
  if vim.tbl_isempty(clients) then
    vim.print("No LSP clients attached"); return
  end

  for _, client in ipairs(clients) do
    local root = (client.config and client.config.root_dir) or "?"
    vim.print(string.format("• %s (id:%d) root:%s", client.name, client.id, root))
  end
end, {})

-- completion helper for attached names
local function attached_names()
  local client_names, processed_names = {}, {}

  for _, clients in ipairs(vim.lsp.get_clients({ bufnr = 0 })) do
    if not processed_names[clients.name] then
      table.insert(client_names, clients.name); processed_names[clients.name] = true
    end
  end
...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(args)
    ...
    local notify_message = "LspAttach triggered ..."
    if vim.wo.diff then
      -- vim.lsp.stop_client(lsp_client_id, true)
      -- vim.lsp.buf_detach_client(buf_id, lsp_client_id)

      notify_message = notify_message .. ", stoping LSP client - " .. lsp_client.name .. "... (for now not really)"
      vim.print(notify_message)
    end
  end,
})
